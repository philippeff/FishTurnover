#*####################################################*#
## Hierarchical model of mean nearest taxon distance  ##
#*####################################################*#
...
time_data_brm <- time_data %>%
  dplyr::select(STUDY_ID, rarefyID, cYEAR, MNTD)
...
## Formula
formula.MNTD <- as.formula('MNTD ~ cYEAR + (1+cYEAR|STUDY_ID) + (1+cYEAR|rarefyID)') # same as DDI paper

## Set priors
# check which parameters can have priors
#get_prior(formula.MNTD, data = time_data_brm, family = brmsfamily('gaussian'))
...
brm_MNTD <- brm(formula.MNTD, 
               family = brmsfamily('lognormal'), # or gaussian??
               data =   time_data_brm,
               prior =  hier_prior_MNTD,
               warmup = 1000, 
               iter = 4000, 
               init = 'random',
...
save(brm_MNTD, file = paste0('./results/brm_MNTD_', st, '.RData'))
